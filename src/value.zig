...
pub const Value = struct {
    ...
    pub fn intToFloatAdvanced(val: Value, arena: Allocator, int_ty: Type, float_ty: Type, mod: *const Module, opt_sema: ?*Sema) !Value {
        const target = mod.getTarget();
        if (int_ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                scalar.* = try intToFloatScalar(elem_val, arena, float_ty.scalarType(mod), target, opt_sema);
            }
    ...
    pub fn intToFloatScalar(val: Value, arena: Allocator, float_ty: Type, mod: *const Module, opt_sema: ?*Sema) !Value {
        const target = mod.getTarget();
        switch (val.tag()) {
    ...
    pub fn intAddSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                scalar.* = try intAddSatScalar(lhs_elem, rhs_elem, ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intSubSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                scalar.* = try intSubSatScalar(lhs_elem, rhs_elem, ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn intMulWithOverflow(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                const of_math_result = try intMulWithOverflowScalar(lhs_elem, rhs_elem, ty.scalarType(mod), arena, target);
    ...
    pub fn intMulSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                scalar.* = try intMulSatScalar(lhs_elem, rhs_elem, ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    /// Supports both floats and ints; handles undefined.
    pub fn numberMax(lhs: Value, rhs: Value, mod: *const Module) Value {
    ...
    /// Supports both floats and ints; handles undefined.
    pub fn numberMin(lhs: Value, rhs: Value, mod: *const Module) Value {
        if (lhs.isUndef() or rhs.isUndef()) return undef;
        if (lhs.isNan()) return rhs;
        if (rhs.isNan()) return lhs;
    ...
    pub fn bitwiseNot(val: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
                var buf: Value.ElemValueBuffer = undefined;
                const elem_val = val.elemValueBuffer(mod, i, &buf);
                scalar.* = try bitwiseNotScalar(elem_val, ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn bitwiseNotScalar(val: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
    ...
    /// operands must be (vectors of) integers; handles undefined scalars.
    pub fn bitwiseAnd(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
    ...
    /// operands must be integers; handles undefined.
    pub fn bitwiseAndScalar(lhs: Value, rhs: Value, arena: Allocator, mod: *const Module) !Value {
        if (lhs.isUndef() or rhs.isUndef()) return Value.initTag(.undef);
    ...
    /// operands must be (vectors of) integers; handles undefined scalars.
    pub fn bitwiseNand(lhs: Value, rhs: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
                var lhs_buf: Value.ElemValueBuffer = undefined;
    ...
    /// operands must be integers; handles undefined.
    pub fn bitwiseNandScalar(lhs: Value, rhs: Value, ty: Type, arena: Allocator, mod: *const Module) !Value {
        if (lhs.isUndef() or rhs.isUndef()) return Value.initTag(.undef);
    ...
    /// operands must be (vectors of) integers; handles undefined scalars.
    pub fn bitwiseOr(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    /// operands must be integers; handles undefined.
    pub fn bitwiseOrScalar(lhs: Value, rhs: Value, arena: Allocator, mod: *const Module) !Value {
        if (lhs.isUndef() or rhs.isUndef()) return Value.initTag(.undef);
    ...
    /// operands must be (vectors of) integers; handles undefined scalars.
    pub fn bitwiseXor(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    /// operands must be integers; handles undefined.
    pub fn bitwiseXorScalar(lhs: Value, rhs: Value, arena: Allocator, mod: *const Module) !Value {
        if (lhs.isUndef() or rhs.isUndef()) return Value.initTag(.undef);
    ...
    pub fn intDiv(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intDivScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        var rhs_space: Value.BigIntSpace = undefined;
    ...
    pub fn intDivFloor(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intDivFloorScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        var rhs_space: Value.BigIntSpace = undefined;
    ...
    pub fn intMod(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intModScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        var rhs_space: Value.BigIntSpace = undefined;
    ...
    pub fn floatRem(lhs: Value, rhs: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
    ...
    pub fn floatMod(lhs: Value, rhs: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intMul(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intMulScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        var rhs_space: Value.BigIntSpace = undefined;
    ...
    pub fn intTrunc(val: Value, ty: Type, allocator: Allocator, signedness: std.builtin.Signedness, bits: u16, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn intTruncBitsAsValue(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                scalar.* = try intTruncScalar(elem_val, allocator, signedness, @intCast(u16, bits_elem.toUnsignedInt(mod)), target);
            }
            return Value.Tag.aggregate.create(allocator, result_data);
        }
        return intTruncScalar(val, allocator, signedness, @intCast(u16, bits.toUnsignedInt(mod)), target);
    }

    pub fn intTruncScalar(val: Value, allocator: Allocator, signedness: std.builtin.Signedness, bits: u16, mod: *const Module) !Value {
        if (bits == 0) return Value.zero;
    ...
    pub fn shl(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn shlScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        const lhs_bigint = lhs.toBigInt(&lhs_space, mod);
    ...
    pub fn shlWithOverflow(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                const of_math_result = try shlWithOverflowScalar(lhs_elem, rhs_elem, ty.scalarType(mod), allocator, target);
                overflowed_data[i] = of_math_result.overflow_bit;
                scalar.* = of_math_result.wrapped_result;
            }
            return OverflowArithmeticResult{
    ...
    pub fn shlSat(
        ...
        if (ty.zigTypeTag(mod) == .Vector) {
            ...
            for (result_data, 0..) |*scalar, i| {
                ...
                const lhs_elem = lhs.elemValueBuffer(mod, i, &lhs_buf);
                const rhs_elem = rhs.elemValueBuffer(mod, i, &rhs_buf);
                scalar.* = try shlSatScalar(lhs_elem, rhs_elem, ty.scalarType(mod), arena, target);
            }
            return Value.Tag.aggregate.create(arena, result_data);
        }
    ...
    pub fn shr(lhs: Value, rhs: Value, ty: Type, allocator: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (ty.zigTypeTag(mod) == .Vector) {
            const result_data = try allocator.alloc(Value, ty.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn shrScalar(lhs: Value, rhs: Value, allocator: Allocator, mod: *const Module) !Value {
        // TODO is this a performance issue? maybe we should try the operation without
        // resorting to BigInt first.
        var lhs_space: Value.BigIntSpace = undefined;
        const lhs_bigint = lhs.toBigInt(&lhs_space, mod);
    ...
    pub fn sqrt(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn sin(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn cos(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn tan(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn exp(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn exp2(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn log(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn log2(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn log10(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn fabs(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn floor(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn ceil(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn round(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub fn trunc(val: Value, float_type: Type, arena: Allocator, mod: *const Module) !Value {
        const target = mod.getTarget();
        if (float_type.zigTypeTag(mod) == .Vector) {
            const result_data = try arena.alloc(Value, float_type.vectorLen());
            for (result_data, 0..) |*scalar, i| {
    ...
    pub const negative_one: Value = .{
        .ip_index = .none,
        .ptr_otherwise = &negative_one_payload.base,
    };
